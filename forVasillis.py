...
for sim in SimList:
    ...
    densNormerSpline = RectBivariateSpline(sim.istep*np.arange(sim.dens.shape[0]),sim.istep*np.arange(sim.dens.shape[1]), sim.dens**(-5./3))
    adiabaticNormer = densNormerSpline(.5*ywidth*(2*np.arange(ybins)+1), .5*xwidth*(2*np.arange(xbins)+1))
    #adiabaticNormer = 1
    ...
    MyHist2 = np.ones(MyHist2.shape)*adiabaticNormer
    ...
    MyeHist2 *= sim.mass_ratio**-1*adiabaticNormer
### MAKE THE PLOTS
